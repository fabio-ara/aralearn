...
- a interface de exemplo passa a manter um único curso de teste, com o primeiro card dedicado a uma tabela com lacunas por bloco/opção
...
- a interface de exemplo passa a expor explicitamente um card de parágrafo com lacunas e uma tabela com lacunas para inspeção manual
...
- `multiple_choice` passa a seguir o padrão visual e estrutural adotado pelo produto, com alternativas em botão, feedback inline e sem dock local de validação
...
- o exemplo de `editor` passa a reunir uma lacuna de digitação e uma lacuna por opção, para validar o comportamento misto no mesmo contêiner
...
- cobertura automatizada para garantir que a interface de exemplo e o exemplo público preservem os novos cenários de laço
...
- cobertura automatizada para a múltipla escolha no padrão visual e estrutural adotado pelo produto
...
- cobertura automatizada para garantir que a interface de exemplo exponha apenas os contêineres esperados, sem `choice` nem `flow`
...
- cobertura automatizada para garantir que a interface de exemplo exponha o editor com lacuna textual e lacuna por opção
...
- o motor geométrico do `flowchart` (`layout` + roteamento + posicionamento de labels) foi substituído por uma implementação literal das regras geométricas de referência, para evitar aproximações por heurística

## [0.9.16] - 2026-05-04

### Added

- exemplos públicos de `flowchart` na interface de exemplo, cobrindo leitura estrutural e prática interativa com preenchimento

### Changed

- o navegador agora versiona novamente o exemplo local para expor automaticamente os cards de teste de fluxograma após a atualização
- a política operacional do projeto passa a exigir exemplos navegáveis sempre que um contêiner novo entrar no `runtime`, incluindo modo de exercício quando aplicável

### Tests

- cobertura automatizada para garantir que a interface de exemplo mantenha cards de fluxograma próprios para checagem manual
...
- a heurística de posicionamento de rótulos do fluxograma foi refinada para reduzir colisões e melhorar casos com retornos e saídas laterais
...
- remoção da trilha ativa do editor manual de cards e contêineres da UI principal, concentrando a autoria estrutural desses contêineres na integração por API
- limpeza da navegação editorial para manter microssequência assistida, leitura, comentários, versões e ações estruturais sem expor o construtor manual anterior
...
- a interface principal do navegador passa a usar o contrato público atual como entrada local
- o exemplo público do navegador passa a nascer no contrato principal do aplicativo

### Tests

- validação automatizada do exemplo usado pela interface principal
...
- módulos, exemplo local, `storage`, CLI e documentação pública renomeados para formas canônicas
...
- interface navegável inicial do AraLearn em `public/`, com entrada local por `index.html`
...
- painel inicial da microssequência para interação com API
- interface inicial de editor de cards separada da leitura
- componentes públicos de UI para home, lição, microssequência, leitura e overlays

### Notes

- esta versão entrega interface e fluxo validável, mas ainda não representa a semântica final do projeto
...
- seleção de card da interface local consolidada em uma rotina única para reduzir duplicação entre leitura, painel da microssequência e editor
...
- refatoração da seleção e da página inicial da interface local para abrir cursos reais do projeto, sem prévias artificiais
...
- configuração local da integração por API no navegador, com persistência da chave e do modelo selecionado
- camada inicial de integração por API via `fetch`, usando saída estruturada em JSON
...
- materialização direta do retorno da API no projeto local, sem depender de resposta fictícia
